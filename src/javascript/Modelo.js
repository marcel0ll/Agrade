...
export default class Modelo {
    constructor(persistencia) {
        ...
        this.carregarListaDeCursosEvent = new Event(this);

        this.pesquisaProcessadaEvent = new Event(this);
        this.itemsModificadosEvent = new Event(this);

        this.cabecalhoModificadoEvent = new Event(this);
    }
    // Função que carrega arquivo json com informações do curso
    carregarCurso(caminho) {
        var _this = this;
        ...
        if (this.listaDeCursos) {
            ...
            $.ajax({
                ...
                success: function(json) {
                    ...
                    _this.listaDeCursos = _this.listaDeCursos.sort(function(
                        a,
                        b
                    ) {
                    ...
                    _this.carregarListaDeCursosEvent.notify(
                        true,
                        _this.listaDeCursos
                    );
                },
    ...
    processarPesquisa(pesquisa) {
        ...
        if (
            typeof this.infoSelected === "undefined" &&
            typeof this.ultimoInfo !== "undefined"
        ) {
            devePesquisar = true;
        }
        if (devePesquisar) {
            ...
            for (var i = 0; i < seletores.length; i++) {
                if (
                    typeof this.curso.disciplinas[0][seletores[i]] ===
                        "undefined" &&
                    typeof this.curso.disciplinas[0].taxonomia[seletores[i]] ===
                        "undefined"
                ) {
            ...
            for (var i = 0; i < seletores.length - 1; i++) {
                ...
                if (
                    typeof this.curso.disciplinas[0].taxonomia[seletor] !==
                    "undefined"
                ) {
            ...
            this.curso.disciplinas = this.curso.disciplinas.sort(function(
                a,
                b
            ) {
                for (var i = 0; i < seletores.length; i++) {
                    ...
                    if (typeof a.taxonomia[seletor] !== "undefined") {
                        if (a.taxonomia[seletor] < b.taxonomia[seletor])
                            return -1;
                        if (a.taxonomia[seletor] > b.taxonomia[seletor])
                            return 1;
                    }
                ...
                if (typeof valores[i] !== "undefined") {
                    ...
                    for (j = 0; j < disciplinas.length; j++) {
                        ...
                        if (typeof disciplina[seletor] !== "undefined") {
                            if (
                                String(disciplina[seletor])
                                    .toLowerCase()
                                    .search(valores[i].toLowerCase()) === -1
                            ) {
                            ...
                            typeof disciplina.taxonomia[seletor] !== "undefined"
                        ) {
                            if (
                                String(disciplina.taxonomia[seletor])
                                    .toLowerCase()
                                    .search(valores[i].toLowerCase()) === -1
                            ) {
                                disciplinas.splice(j, 1);
            ...
            if (taxonomias.length > 0) disciplinasObj = {};
            else disciplinasObj = [];
                ...
                if (taxonomias.length > 0) {
                    for (var i = 0; i < taxonomias.length; i++) {
                        ...
                        if (
                            ...
                            var chave =
                                seletor + " " + disciplina.taxonomia[seletor];
    ...
    prepararDadosDeCurso(curso) {
        ...
        curso.disciplinas.forEach(function(disciplina) {
            ...
            listaDeConjuntosComCreditosNaoCalculados.forEach(function(
                conjunto
            ) {
    ...
    processarCabecalho(notify) {
        ...
        this.curso.disciplinas.forEach(function(disciplina) {
            if (disciplina.liberada && disciplina.feita) {
                ...
                if (typeof conjunto !== "undefined") {
                    if (
                        conjunto.disciplinas <= 0 ||
                        conjunto.disciplinasFeitas < conjunto.disciplinas
                    ) {
        ...
        this.curso.conjuntos.forEach(function(conjunto) {
            ...
            _this.curso.totalDeCreditosFeitos += Math.min(
    ...
    interagirComInfo(id) {
        ...
        if (typeof this.infoSelected !== "undefined") {
            ...
            disciplinas.desbloqueia = $.grep(this.curso.disciplinas, function(
                e
            ) {
                return $.inArray(id, e.requisitos) !== -1;
            });
